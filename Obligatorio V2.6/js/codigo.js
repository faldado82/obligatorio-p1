...
let btnBuscadorEjercicios = `<label>Buscar Ejercicios planteados por tu docente (${usuario.Docente.nombre}):</label><input type="search" class="inputBusqueda" id="inputBusquedaTxt" placeholder="Busque su ejercicio por título o descripción"> <button id="buscaEjercicio">Buscar</button>`

let resultadoBusquedaDiv = `<div id="resultadoBusqueda">Aca van los resultados</div>`
...
document.querySelector('#seccion1').innerHTML = btnBuscadorEjercicios + resultadoBusquedaDiv + tablaEjercicios;

// FIXME: ARREGLAR BUSQUEDA
// Le agregamos un evento al boton de busqueda
document.querySelector('#buscaEjercicio').addEventListener('click', function(){
let mensajeDeBusqueda = '';

let tablaResultados = `<table><tr><th>Título del Ejercicio </th><th>Descripción</th><th>Imagen</th></tr>`;
  ...
  if(inputBusquedaTxt.trim().length > 0){
    ...
    for(let i = 0; i < ejercicios.length; i++){
      ...
      let imgResult = `<img src="Ejercicios/img/${unEjercicio.imagen}" >`
      if(unEjercicio.Docente.usuario === docenteDelAlumno.usuario && unEjercicio.Nivel === usuario.Nivel){
    
        if(unEjercicio.tituloEjercicio.indexOf(inputBusquedaTxt) !== -1){

          mensajeDeBusqueda = 'Busqueda exitosa' 

          mensajeDeBusqueda +=  
          tablaResultados+=`<tr><td>${unEjercicio.tituloEjercicio}</td><td>${unEjercicio.descripcionEjercicio}</td><td>${imgResult}</td></tr>`
          


        } else {

          mensajeDeBusqueda = 'No se encontraron resultados'
        }
  ...
  document.querySelector('#resultadoBusqueda').innerHTML = mensajeDeBusqueda
