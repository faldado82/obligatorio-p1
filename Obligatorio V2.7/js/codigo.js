...
// FIXME: terminar seccion 3
function seccion3Docente() {
  let tabla = `<table><th>Nombre alumno</th><th>Ejercicio</th><th>audio</th><th>Redacte devolución</th><th>Enviar entrega</th>`;
  ...
  for (let i = 0; i < entregas.length; i++) {
    ...
    tabla += `<tr><td>${unaEntrega.Alumno.nombre}</td><td>${
      unaEntrega.Ejercicio.tituloEjercicio
    }</td><td>${sonido}</td><td><input type="text" id="devolucionInput${accumulador1++}" placeholder="Escriba devolución"</td> <td><button id="enviarDevoluciondevolucionInput${accumulador2++}">Enviar devolución</button></td></tr>`;
  }

  tabla += `</table>`;

  document.querySelector('#seccion3').innerHTML = tabla;
}
