...
function seccion4Docente() {
  ...
  let alumnoQueMasResolvio = null;

  // mostrar cantidad total de ejercicios entregados por docente
  for (let i = 0; i < entregas.length; i++) {
    ...
    if (unaEntrega.Alumno.cantidadEntregas > mayorCantidadDeEjResueltos) {
      mayorCantidadDeEjResueltos = unaEntrega.Alumno.cantidadEntregas;
      alumnoQueMasResolvio = unaEntrega.Alumno;
    }
  }

  // Mostrar nombre alumnos

  // FIXME: si son varios mostrar varios nombres
  // Encontramos el maximo de cantidad de entregas, y luego recorrer y guardar todos los alumnos que tienen esa misma cantidad.

  document.querySelector('#seccion4').innerHTML =
    titulo +
    `<p>Le han entregado ${contadorTotalTareasEntregadas} tareas en total</p> <br> El alumno que mas entregó fue: ${alumnoQueMasResolvio.nombre} (${alumnoQueMasResolvio.usuario}) con un total de ${mayorCantidadDeEjResueltos}`;
}
...
function seccion3Alumno() {
  let titulo = `<h2>Ver ejercicios resueltos</h2>`;
  let tabla = `<table id="tablaTareasResueltas" class="tablaDevoluciones"><th>Título del Ejercicio</th><th>Descripción</th><th>Sonido</th><th>Devolución del docente</th>`;
  ...
  for (let i = 0; i < entregas.length; i++) {
    ...
    if (
      ...
      tabla += `<tr><td>${unaEntrega.Ejercicio.tituloEjercicio}</td><td>${unaEntrega.Ejercicio.descripcionEjercicio}</td> <td>${sonido}</td><td>${unaEntrega.devolucion}</td></tr>`;
    } else if (
      unaEntrega.Alumno.usuario === cuentaActiva.usuario &&
      unaEntrega.devolucion === ''
    ) {
      tabla += `<tr><td>${unaEntrega.Ejercicio.tituloEjercicio}</td><td>${unaEntrega.Ejercicio.descripcionEjercicio}</td> <td>${sonido}</td><td>Aún el docente no redactó una devolución</td></tr>`;
    }
  }
