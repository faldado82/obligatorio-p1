// TODO: Ocultar todas las secciones, solo aparecen cuando se apretan en el nav bar
...
function seccion3Docente() {
  ...
  let tabla = `<table id="tablaTareasPorCorregir" class="tablaDevoluciones" ><th>Nombre alumno</th><th>Ejercicio</th><th>Imagen</th><th>Audio del alumno</th><th>Redacte devolución</th><th>Enviar entrega</th>`;
  ...
  for (let i = 0; i < usuarios.length; i++) {
    ...
    if (unUsuario.tipo === 'alumno') {
      for (let j = 0; j < unUsuario.entregasIndividuales.length; j++) {
        unaEntrega = unUsuario.entregasIndividuales[j];
        ...
        if (
          ...
          tabla += `<tr data-id="${unaEntrega.id}"><td>${unaEntrega.Alumno.nombre}</td><td>${unaEntrega.Ejercicio.tituloEjercicio}</td><td>${img}</td><td>${sonido}</td><td><input type="text" placeholder="Escriba devolución" class="inputDevolucion" /> </td> <td><button class="btnFila">Enviar devolución</button></td></tr>`;
        }
      }
    }
  }
...
function seccion4Docente() {
  ...
  // Mostrar cantidad total de ejercicios entregados en total
  ...
  document.querySelector('#seccion4').innerHTML =
    titulo +
    `<p>El total de tareas entregadas por todos sus alumnos es: <b> ${contadorTotalTareasEntregadas}</b>
    <br> El o los alumnos que más resolvieron fueron: ${alumnoQueMasResolvio} </p>` +
    selectAlumnos +
    divMensaje;
...
function mostrarEstadisticasIndividuales() {
  ...
  if (alumnoSeleccionado !== null) {
    let resueltos = alumnoSeleccionado.entregasIndividuales.length;
    ...
    document.querySelector(
      '#mensajeEstadisticasIndividuales'
    ).innerHTML = `<p>El total de ejercicios planteados a ${alumnoSeleccionado.nombre} fue:<b> ${todosLosEjPlanteados} </b> <br> De todos estos ha resuelto <b> ${resueltos} </b>ejercicios.</p>`;
  }
}
...
function seccion3Alumno() {
  ...
  for (let i = 0; i < cuentaActiva.entregasIndividuales.length; i++) {
    ...
    if (
      ...
      tabla += `<tr><td>${unaEntrega.Ejercicio.tituloEjercicio}</td><td>${unaEntrega.Ejercicio.descripcionEjercicio}</td><td>${img}</td><td>${sonido}</td><td>${unaEntrega.devolucion}</td></tr>`;
    } else if (
      unaEntrega.Alumno.usuario === cuentaActiva.usuario &&
      unaEntrega.devolucion === ''
    ) {
      tabla += `<tr><td>${unaEntrega.Ejercicio.tituloEjercicio}</td><td>${unaEntrega.Ejercicio.descripcionEjercicio}</td><td>${img}</td> <td>${sonido}</td><td>Aún el docente no redactó una devolución</td></tr>`;
    }
  }
...
function seccion4Alumno() {
  ...
  let porcentaje = (totalResueltos * 100) / totalEjPlanteados;

  divMensaje = `<div><p>Porcentaje de ejercicios resueltos:<b> ${porcentaje}%</b><br>Total de planteados: <b>${totalEjPlanteados}</br></p></div>`;
